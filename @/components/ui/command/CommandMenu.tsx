        ...
        : results?.commands.map((category, index) => (
            <div key={index}>
              {/* {category.commands.length > 0 && (
                <p className="text-sm text-scale-10">{category.category}</p>
              )} */}
...
const Wrapper: FC<MenuProps & { children: ReactNode }> = (props) => {
  ...
  return (
    <motion.div
      className="flex flex-col w-full overflow-y-auto overflow-x-hidden transition-height duration-100 ease-linear will-change-height"
      role="listbox"
      style={{
